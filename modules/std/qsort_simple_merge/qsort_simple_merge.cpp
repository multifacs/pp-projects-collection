...
#include "../../../modules/std/qsort_simple_merge/qsort_simple_merge.h"
...
INT_VEC getRandomVec(int n) {  // Radix
...
void quickSortSimpleMerge(INT_VEC* vec, int parts, bool parallel) {
  ...
  if (parallel) {
    ...
    for (int i = 0; i < parts; i++) {
      threads.push_back(std::thread(
          [&vecs, i]() { quickSort(vecs[i].begin(), vecs[i].end()); }));
    }

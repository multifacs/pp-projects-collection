...
void measureTime(INT_VEC* vec1, INT_VEC* vec2, int parts, bool parallel) {
  ...
  start = omp_get_wtime();
  radixSortSimpleMerge(vec2, parts, parallel);
  end = omp_get_wtime();
...
TEST(QUICK_SORT_SIMPLE_MERGE_TBB, TEST_1) {
  int n = 10;
  int parts = 3;
...
TEST(QUICK_SORT_SIMPLE_MERGE_TBB, TEST_2) {
  int n = 11;
  int parts = 11;
  INT_VEC vec = getRandomVec(n);
  radixSortSimpleMerge(&vec, parts);
...
TEST(QUICK_SORT_SIMPLE_MERGE_TBB, TEST_3) {
  int n = 12;
  int parts = 3;
  INT_VEC vec = getRandomVec(n);
  radixSortSimpleMerge(&vec, parts);
...
TEST(QUICK_SORT_SIMPLE_MERGE_TBB, TEST_4) {
  int n = 13;
  int parts = 3;
  INT_VEC vec = getRandomVec(n);
  radixSortSimpleMerge(&vec, parts);
...
TEST(QUICK_SORT_SIMPLE_MERGE_TBB, TEST_5) {
  int runs = 3;
  int n = 100;

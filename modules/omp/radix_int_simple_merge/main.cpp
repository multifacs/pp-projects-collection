...
TEST(RADIX_INT_SIMPLE_MERGE_OMP, TEST_1) {
  int n = 10;
  int parts = 3;
...
TEST(RADIX_INT_SIMPLE_MERGE_OMP, TEST_2) {
  int n = 11;
  int parts = 11;
  INT_VEC vec = getRandomVec(n);
  radixSortSimpleMerge(&vec, parts);
...
TEST(RADIX_INT_SIMPLE_MERGE_OMP, TEST_3) {
  int n = 12;
  int parts = 3;
  INT_VEC vec = getRandomVec(n);
  radixSortSimpleMerge(&vec, parts);
...
TEST(RADIX_INT_SIMPLE_MERGE_OMP, TEST_4) {
  int n = 13;
  int parts = 3;
  INT_VEC vec = getRandomVec(n);
  radixSortSimpleMerge(&vec, parts);
